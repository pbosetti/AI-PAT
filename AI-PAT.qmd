...
* **realizzare** la procedura si chiama **addestramento**
...
p2 <- p0 + geom_point(data=filter(df, s2==FALSE), color="orange")
```
...
:::column
```{r fig.cap="Due possibili modelliverificati su un nuovo"}
{p2 + geom_line(aes(y=m1))} /
{p2 + geom_line(aes(y=m2))} 
```
:::
:::
...
* Rosenblatt (1958) hs dato la prima formulazione di una rete basata su collegamenti tra neuroni (*perceptron*)
* Reti LSTM proposte dal Hochreiter nel 1995
* Andrew Ng, addestramento di CNN su GPU, 2009
:::
