...
df <- rbind(male, female) %>% 
  mutate(
    train = sample(c(T,F), size=n(), replace=T, prob=c(80,20)),
    maschio = ifelse(gender == "maschio", 1, 0),
    femmina = ifelse(gender == "femmina", 1, 0),
  ) %>% 
  rename(statura=height, peso=mass)

set.seed(0)
model = neuralnet(
  maschio+femmina~peso + statura,
  data = filter(df, train) %>% dplyr::select(-gender),
  hidden = c(3),
  rep=1,
  linear.output = FALSE,
  lifesign="full",
...
* *Machine learning* significa realizzare ed usare una procedura di calcolo (**algoritmo**) che fornisce predizioni sulla base di **un'esperienza**
...
:::column
```{r}
dplyr::select(df, !train) %>% 
  bind_cols(
    predict(model, .) %>% 
      `colnames<-`(c("maschio_p", "femmina_p")) %>% 
      as_tibble()
  ) %>% 
  mutate(
    gender_p = ifelse(maschio_p > femmina_p, "maschio", "femmina"),
    correct = gender == gender_p, 
    label = ifelse(correct, gender, "errore"),
    label = factor(label, ordered=T, levels=c("femmina", "maschio", "errore"))
  ) %>% 
  arrange(label) %>% 
  ggplot(aes(x=statura, y=peso)) +
  geom_point(aes(color=label, shape=gender)) + 
  labs(x="Statura (cm)", y="Peso (kg)", color="Inferenza", shape="Sesso") + 
  scale_color_discrete(palette=c("pink", "lightblue", "red"))
```
:::
